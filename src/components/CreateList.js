...
function CreateList() {
  ...
  const [selectedMovies, setSelectedMovies] = useState([]); // Updated state for selected movies
  const [selectedMovieIds, setSelectedMovieIds] = useState([]);
  const moviesPerRow = 5; // Number of movies to show per row
  const [list, setList] = useState(false);
  const [listTitle, setListTitle] = useState(""); // State for list name
  ...
  const saveData = ()=>{
    ...
    fetch("https://moviegeek.azurewebsites.net/listStatic/addDAO", {
          ...
          body: JSON.stringify({listName: listTitle, listMovieID: selectedMovieIds, listOwner: username, listDescript: descript}),
        })
  ...
  const handleMovieSelect = (movie) => {
    const isAlreadySelected = selectedMovies.some((selectedMovie) => selectedMovie.id === movie.id);
  if (!isAlreadySelected) {
    setSelectedMovies((prevMovies) => [...prevMovies, movie]);
    // setSelectedMovieIds((prevIds) => [...prevIds, movie.id]);
    setSearchInput(""); // Clear the search input after selecting a movie
    // alert(selectedMovieIds)
  }
  };
    ...
    <div className="display-movie">
          {chunkMovies(selectedMovies, moviesPerRow).map((row, rowIndex) => (
            <div className="movie-row" key={rowIndex}>
              {row.map((movie) => (
                <MovieCard key={movie.id} imageUrl={movie.poster} />
              ))}
            </div>
          ))}
        </div>
